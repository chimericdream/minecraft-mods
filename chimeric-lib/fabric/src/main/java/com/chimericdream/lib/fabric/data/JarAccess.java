...
public final class JarAccess {
    private static final Logger LOGGER = LogManager.getLogger(JarAccess.class);
    ...
    private JarAccess() {
        if (this.path == null) {
            LOGGER.warn("The '{}' environment variable is unset, some generators may not load.", ENV_KEY);
        }
    }
    ...
    public static Optional<JarEntry> getJarEntry(String path, boolean allowDirs) {
        ...
        if (entry == null) {
            LOGGER.warn("Unable to load entry '{}'", path);
