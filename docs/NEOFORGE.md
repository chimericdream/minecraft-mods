...
> ⚠ **The lambda indices in that worked example were derived against MC 26.2 + its NeoForge
> build.** The *pattern* — NeoForge lifting a method body into a synthetic lambda and
> shifting every later lambda's index — is what carries across versions; the specific
> `$0`/`$1`/`$2` numbers do not. Re-derive them with `javap -p -c` against the patched jar
> for whichever MC/NeoForge pair you are building against.
