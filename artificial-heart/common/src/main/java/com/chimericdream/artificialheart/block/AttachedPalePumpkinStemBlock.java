...
public class AttachedPalePumpkinStemBlock extends AttachedStemBlock {
    public static final Identifier BLOCK_ID = Identifier.fromNamespaceAndPath(ModInfo.MOD_ID, "attached_pale_pumpkin_stem");
    public static final ResourceKey<Block> BLOCK_REGISTRY_KEY = ResourceKey.create(Registries.BLOCK, BLOCK_ID);

    public AttachedPalePumpkinStemBlock() {
        super(
            PalePumpkinStemBlock.BLOCK_REGISTRY_KEY,
            PalePumpkinBlock.BLOCK_REGISTRY_KEY,
            PalePumpkinSeedsItem.ITEM_REGISTRY_KEY,
            BlockTags.SUPPORTS_PUMPKIN_STEM,
            Properties.ofFullCopy(Blocks.ATTACHED_PUMPKIN_STEM).setId(BLOCK_REGISTRY_KEY)
        );
    }
}
