...
/**
 * Emits a single textured quad for one face of a hand-cut, axis-aligned box. Used by
 * {@code DecoratedPumpkinStencilRenderer} to draw a carved stencil overlay directly onto one face of a
 * decorated pumpkin. Mirrors {@code log-all-the-things}' own copy of this class (this mod has no shared
 * dependency on that one, so the small amount of vertex math is duplicated rather than pulled in cross-mod).
 */
public final class QuadEmitter {
    /**
     * Two coplanar quads at the same depth z-fight (flicker between which one wins per pixel) — a
     * stencil overlay is always drawn flush against the pumpkin's own real face. Nudging every vertex
     * outward along its own face normal by a sliver moves the overlay just in front of the coincident
     * host face, resolving the tie without any visible gap.
     */
