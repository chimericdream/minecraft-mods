 ...
 */
public record DyedPumpkinItemTintSource(int defaultColor) implements ItemTintSource {
    public static final Identifier ID = Identifier.fromNamespaceAndPath(ModInfo.MOD_ID, "dyed_pumpkin");

    public static final MapCodec<DyedPumpkinItemTintSource> MAP_CODEC = RecordCodecBuilder.mapCodec(i -> i.group(
        ExtraCodecs.RGB_COLOR_CODEC.fieldOf("default").forGetter(DyedPumpkinItemTintSource::defaultColor)
    ).apply(i, DyedPumpkinItemTintSource::new));
    ...
    @Override
    public MapCodec<DyedPumpkinItemTintSource> type() {
        return MAP_CODEC;
    }
}
