...
On 26.1.2 `Minecraft.screen` is still a public field — gate on
`mc.screen instanceof TitleScreen`. (`Gui` has no `screen()` accessor on this version; the
`mc.gui.screen()` form belongs to 26.2, where the `mc.screen` field is gone.)
...
- On exit you will likely see a `java.lang.Error: Watchdog (Client shutdown from post-main)` crash —
  **ignore it, for either procedure above.** It's a known non-daemon thread leak in a shared
  dependency, not your code or a real failure. The game did fully close.
