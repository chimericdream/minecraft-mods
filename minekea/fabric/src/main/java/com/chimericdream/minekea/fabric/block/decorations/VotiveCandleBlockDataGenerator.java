...
//            .offerTo(exporter, BLOCK.BLOCK_ID.withSuffixedPath("_universal_dyeing").toString());
    ...
    public void configureBlockStateModels(BlockStateModelGenerator blockStateModelGenerator) {
        ...
        WeightedVariant oneCandle = BlockStateModelGenerator.createWeightedVariant(oneCandleModelId);
        WeightedVariant oneLitCandle = BlockStateModelGenerator.createWeightedVariant(oneCandleLitModelId);
        WeightedVariant twoCandles = BlockStateModelGenerator.createWeightedVariant(twoCandlesModelId);
        WeightedVariant twoLitCandles = BlockStateModelGenerator.createWeightedVariant(twoCandlesLitModelId);
        WeightedVariant threeCandles = BlockStateModelGenerator.createWeightedVariant(threeCandlesModelId);
        WeightedVariant threeLitCandles = BlockStateModelGenerator.createWeightedVariant(threeCandlesLitModelId);
        WeightedVariant fourCandles = BlockStateModelGenerator.createWeightedVariant(fourCandlesModelId);
        WeightedVariant fourLitCandles = BlockStateModelGenerator.createWeightedVariant(fourCandlesLitModelId);

        blockStateModelGenerator.blockStateCollector
            .accept(
                VariantsBlockModelDefinitionCreator.of(BLOCK)
                    .with(
                        BlockStateVariantMap.models(VotiveCandleBlock.CANDLES, VotiveCandleBlock.LIT)
                            .register(1, false, oneCandle)
                            .register(1, true, oneLitCandle)
                            .register(2, false, twoCandles)
                            .register(2, true, twoLitCandles)
                            .register(3, false, threeCandles)
                            .register(3, true, threeLitCandles)
                            .register(4, false, fourCandles)
                            .register(4, true, fourLitCandles)
                    )
            );
    }
}
